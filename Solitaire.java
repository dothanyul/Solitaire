...
public class Solitaire {
	...
	public void up(int stackNum) throws IOException {
		...
		if (card.number() == 1) {  // an ace can always go up
			...
			throw new IOException(" Cards in foundation must be in sequence.");
		}
	}
	...
	public void up() throws IOException {
		...
		if (card.number() == 1) {
			...
			throw new IOException(" Foundation piles must start with aces.");
		} else if (top[card.suit()].peek().number() == card.number()-1) {  // if the stack this card needs to go on has the correct previous card already on it
			top[card.suit()].push(card);  // put it up
			card.flip();  // it was flipping over for some reason
			flip.top();  // take the card off the flip stack
	...
	public void move(int to) throws IOException {
		if (flip.isEmpty()) {
			throw new IOException(" Can't move from empty stock.");
		}
	...
	public static void main(String[] args) {
		...
		if (args.length > 0 && args[0].equals("-h")) {
			System.out.println(" This is a command line solitaire game. It lets you play solitaire on the\ncommand line.");
			System.out.println(" Enter a tableau stack number 1-7 or \"s\" for the stock for your move's origin,\nthen enter a tableau stack number 1-7 for your move's destination or \"b\" to\nbuild the card onto a foundation pile. Enter \"f\" to flip through the stock.\nEnter \"quit\" at any time to give up.");
			...
			while (! game.won()) {
				...
				if (! c.hasNext()) {
					...
					if (word.equals("f")) {
						...
						game.setError("Enter a tableau pile number 1-7 or \"s\" for the\n"
								+ "stock for your move's origin, then enter another\n"
								+ "pile number for your move's destination or \"b\"\n"
								+ "to build the card onto a foundation pile. Enter\n"
                                + "\"f\" to flip through the stock. Enter \"quit\" at\n"
                                + "any time to give up.");
						continue;  // skip the two-word commands
				...
				if (orig.equals("s") && dest.equals("b")) {
					try {
						game.up();
					} catch (IOException e) {
					...
					int to = 0;
					int from = 0;
					// get the pile numbers if they aren't stack or foundation
					if (! orig.equals("s")) {
						try {
							...
							game.setError(" Enter a valid origin.");
							continue;
						}
					}
					if (! dest.equals("b")) {
		    ...
		    for (int i = 0; i <= 9; i++) {
			    System.out.print("\033[1A\033[2K"); // move up one line, delete current line
		    }
